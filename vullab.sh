...
#to be considered a succesfull detecton
match_window="5"
...
if [ "$help" = "true" ]; then
	...
	Usage: $(basename "$0") [--parallel NUMBER_OF_CORES] [--solidifi]
                            ...
                            [--analyse NUMBER_OF_CONTRACTS] [--gpt NUMBER_OF_CONTRACTS]
							[--help]
	...
	Options:
	  ...
	  -t, --threads N        Configures how many threads will run simutaniouly during insertion and detection phases of the execution. (deafult: 1).
	  
	  -c, --cpus			 Configures how many CPUs will be assigned to each proccess during insertion and detection phases of the execution. (default: 1).
	  ...
	  -a, --analyse N        Executes SmartBugs 2.0 in the selected number of .sol files in $(basename "$PWD")/dataset/manual_dataset.
	  						 Takes as optional argument the quatity of contracts for vulnerability in which SmartBugs 2.0 will be executed for each vulnerability. (default: all).
	  ...
	  -g, --gpt N S          Executes GPT as a state-of-the-art vulnerability detection tool in the selected number of .sol files in $(basename "$PWD")/dataset/manual_dataset via OpenAI's API.
	                         Takes as optional argument the quatity of contracts for vulnerability  in which GPT will be executed for each vulnerability. (default: all).
							 The contracts are sorted by line count considering only functional lines.
	...
	Examples:
	  $(basename "$0") -s --analyse                     Executes SolidiFI. Also executes Smartbugs 2.0 in all inserted contracts.
	...
	Notes:
	  ...
	  - We recommend using the flgas in the following order -t, -c, -u, -s, -a, -g, -m, -b. 
	    You can also use them separatelly, calling $(basename "$0") several times.
